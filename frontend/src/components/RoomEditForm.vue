...
export default {
  ...
  mounted() {
    this.gatewayClient.getRoom(this.$route.params.roomUuid, function (error, response) {
      if (error) {
        ...
        this.ruleForm.roomType = response.getRoomtype()
        this.ruleForm.amount = response.getAmount()
        this.ruleForm.beds = response.getBeds()
        this.ruleForm.nightPrice = response.getNightprice()
        this.ruleForm.offers = response.getOffersList()
      }
    }.bind(this))
  },
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const roomUuid = this.$route.params.roomUuid
          this.gatewayClient.patchRoom({
            room: {
              hotelUuid: hotelUuid,
              roomUuid: roomUuid,
